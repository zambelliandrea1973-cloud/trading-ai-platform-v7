...
import { useGetAssetAnalysis, getGetAssetAnalysisQueryKey, useGetBrokerStatus, getGetBrokerStatusQueryKey, useGetDashboard, useGetMarkets, useGetOpportunities, useHealthCheck, useGetNews, getGetNewsQueryKey, useGetAxiProgress, useGetAxiRules, useGetV72Opportunities, useCreateDecisionMemory, useUpdateDecisionMemoryOutcome, useGetRecentDecisionMemory, getGetRecentDecisionMemoryQueryKey, type AssetAnalysis, type Dashboard, type Market, type Opportunity, type GetNewsParams, type RankedOpportunity } from '@workspace/api-client-react';
...
function RankedOpportunityCard({ opportunity, index }: { opportunity: RankedOpportunity; index: number }) {
  const { t } = useI18n();
  const actionable = opportunity.decision === 'BUY' || opportunity.decision === 'SELL';
  return <Link href={`/assets/${encodeURIComponent(opportunity.symbol)}`} className={`panel panel-hover block p-5 no-underline fade-up delay-${Math.min(index + 1, 3)}`} data-testid={`card-v72-opportunity-${opportunity.symbol}`}>
    <div className="mb-4 flex items-start justify-between gap-3"><div><span className="mono text-lg font-medium text-foreground">{opportunity.symbol}</span><p className="mt-1 text-xs text-muted-foreground">{t('opps.opportunityScore')} {opportunity.opportunityScore.toFixed(1)}</p></div><Badge tone={actionable ? 'positive' : opportunity.decision === 'NO_TRADE' ? 'negative' : 'amber'}>{opportunity.decision.replace('_', ' ')}</Badge></div>
    <div className="grid grid-cols-2 gap-3 rounded-md bg-secondary/30 p-3"><div><p className="eyebrow">{t('opps.protection')}</p><p className="mono mt-1 text-[11px] text-primary">{opportunity.protectionMode.replaceAll('_', ' ')}</p></div><div><p className="eyebrow">{t('opps.marketRegime')}</p><p className="mono mt-1 text-[11px] text-foreground">{opportunity.marketRegime}</p></div><div><p className="eyebrow">{t('opps.finalScore')}</p><p className="mono mt-1 text-sm text-foreground">{opportunity.finalScore ?? '—'}</p></div><div><p className="eyebrow">{t('opps.paperSize')}</p><p className="mono mt-1 text-sm text-accent">{Math.round(opportunity.sizeMultiplier * 100)}%</p></div></div>
    <div className="mt-4 border-t border-border pt-4"><p className="eyebrow mb-2">{t('opps.reasons')}</p>{opportunity.reasons.length ? <ul className="space-y-1">{opportunity.reasons.map((reason) => <li className="text-xs leading-relaxed text-muted-foreground" key={reason}>• {reason}</li>)}</ul> : <p className="text-xs text-muted-foreground">{t('opps.noExtraGuards')}</p>}</div>
  </Link>;
}
...
export function DashboardPage() {
  ...
  const axiProgressQuery = useGetAxiProgress();
  const axiRulesQuery = useGetAxiRules();
  const axiProgress = axiProgressQuery.data;
  const activeAxiRules = axiProgress ? axiRulesQuery.data?.stages[axiProgress.stage] : undefined;
  ...
  return <div className="content-wrap">
    ...
    <section className="mb-6 panel p-5 md:p-6" data-testid="axi-paper-progress">
      <SectionLabel aside={<Badge tone="teal">{t('axi.paperOnly')}</Badge>}>{t('axi.title')}</SectionLabel>
      {axiProgressQuery.isLoading || axiRulesQuery.isLoading ? <StateMessage kind="loading" title={t('axi.loading')} body="" /> : axiProgress && activeAxiRules ? <><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label={t('axi.stage')} value={axiProgress.stage.replaceAll('_', ' ')} detail={`${t('axi.edgeScore')} ${axiProgress.edgeScore ?? '—'}`} tone="amber" /><Metric label={t('axi.protectionMode')} value={axiProgress.protectionMode.replaceAll('_', ' ')} detail={`${t('axi.sizeMultiplier')} ${Math.round(axiProgress.baseSizeMultiplier * 100)}%`} tone={axiProgress.protectionMode === 'NORMAL' ? 'positive' : 'amber'} /><Metric label={t('axi.marketRegime')} value={axiProgress.marketRegime} detail={`${t('axi.closedTrades')} ${axiProgress.closedTrades ?? '—'}`} /><Metric label={t('axi.progress')} value={`${axiProgress.stageProfitPct?.toFixed(2) ?? '—'}%`} detail={`${t('axi.target')} ${activeAxiRules.profitTargetPct ?? '—'}%`} /></div><div className="mt-4 grid gap-3 border-t border-border pt-4 sm:grid-cols-4"><div><p className="eyebrow">{t('axi.minEquity')}</p><p className="mono mt-1 text-xs text-foreground">${activeAxiRules.minEquityUsd.toLocaleString()}</p></div><div><p className="eyebrow">{t('axi.minTrades')}</p><p className="mono mt-1 text-xs text-foreground">{activeAxiRules.minTrades ?? '—'}</p></div><div><p className="eyebrow">{t('axi.minDays')}</p><p className="mono mt-1 text-xs text-foreground">{activeAxiRules.minDays ?? '—'}</p></div><div><p className="eyebrow">{t('axi.maxLoss')}</p><p className="mono mt-1 text-xs text-destructive">{activeAxiRules.maxLossPct}%</p></div></div></> : <StateMessage kind="error" title={t('axi.unavailable')} body={t('error.desc')} onRetry={() => { axiProgressQuery.refetch(); axiRulesQuery.refetch(); }} />}
    </section>
...
export function OpportunitiesPage() {
  ...
  const query = useGetV72Opportunities({ locale });
  const [mode, setMode] = useState(t('opps.all'));
  const filters = [t('opps.all'), 'BUY', 'SELL', 'WAIT', 'NO TRADE'];
  const filtered = mode === t('opps.all') ? query.data ?? [] : (query.data ?? []).filter((o) => o.decision.replace('_', ' ') === mode);
  return <div className="content-wrap"><PageHeader eyebrow={t('opps.eyebrow')} title={t('opps.title')} subtitle={t('opps.subtitle')} action={<Badge tone="amber"><Brain size={11} />V7.2 {t('opps.decisionEngine')}</Badge>} /><div className="mb-5 flex flex-wrap gap-2">{filters.map((item) => <button key={item} onClick={() => setMode(item)} className={`rounded-md px-3 py-2 text-xs font-semibold ${mode === item ? 'bg-primary text-primary-foreground' : 'border border-border text-muted-foreground hover:text-foreground'}`} data-testid={`button-opportunity-filter-${item.toLowerCase().replaceAll(' ', '-')}`}>{item}</button>)}</div>{query.isLoading ? <StateMessage kind="loading" title={t('opps.title')} body="" /> : query.isError ? <StateMessage kind="error" title={t('opps.unavailable')} body={t('error.desc')} onRetry={() => query.refetch()} /> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filtered.map((item, index) => <RankedOpportunityCard opportunity={item} index={index} key={item.symbol} />)}</div>}<div className="mt-6 rounded-lg border border-border bg-secondary/30 p-4 text-xs text-muted-foreground"><Info size={14} className="mr-2 inline text-primary" />{t('opps.agreement')}</div></div>;
}
...
export function AssetPage() {
  ...
  return <div className="content-wrap">
    ...
    {query.isLoading ? <StateMessage kind="loading" title={t('markets.loading')} body="" /> : query.isError && !asset ? <StateMessage kind="error" title={t('error.title')} body={t('error.desc')} onRetry={() => query.refetch()} /> : asset ? <>
      ...
      <SectionLabel aside={<Badge tone={asset.masterDecision.hardVeto ? 'negative' : 'teal'}>{asset.masterDecision.hardVeto ? t('asset.hardVetoActive') : t('asset.paperDecision')}</Badge>}>{t('asset.masterDecision')}</SectionLabel>
      <div className="mb-6 grid gap-4 lg:grid-cols-[.8fr_1.2fr]">
        <section className="panel p-5" data-testid="master-decision-panel"><div className="flex items-start justify-between"><div><p className="eyebrow">{t('asset.finalScore')}</p><p className="mono mt-2 text-4xl text-primary">{asset.masterDecision.finalScore ?? '—'}</p></div><Badge tone={asset.masterDecision.decision === 'BUY' ? 'positive' : asset.masterDecision.decision === 'SELL' || asset.masterDecision.decision === 'NO_TRADE' ? 'negative' : 'amber'}>{asset.masterDecision.decision.replace('_', ' ')}</Badge></div><div className="mt-5 grid grid-cols-2 gap-3 border-t border-border pt-4"><div><p className="eyebrow">{t('common.confidence')}</p><p className="mono mt-1 text-sm text-foreground">{asset.masterDecision.confidence}%</p></div><div><p className="eyebrow">{t('asset.sizeMultiplier')}</p><p className="mono mt-1 text-sm text-accent">{Math.round(asset.masterDecision.sizeMultiplier * 100)}%</p></div></div><p className="mt-4 text-xs leading-relaxed text-muted-foreground">{asset.masterDecision.rationale}</p></section>
        <section className="panel p-5"><SectionLabel>{t('asset.safetyGuards')}</SectionLabel>{asset.masterDecision.hardVetoReasons.length > 0 && <div className="mb-3"><Notice tone="negative"><ul>{asset.masterDecision.hardVetoReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></Notice></div>}{asset.masterDecision.softGuards.length > 0 ? <ul className="space-y-2">{asset.masterDecision.softGuards.map((guard) => <li className="flex gap-2 rounded-md bg-secondary/40 p-3 text-xs text-muted-foreground" key={guard}><ShieldAlert size={14} className="shrink-0 text-primary" />{guard}</li>)}</ul> : <Notice tone="teal"><span>{t('asset.noSoftGuards')}</span></Notice>}<div className="mt-4 grid grid-cols-2 gap-2 border-t border-border pt-4 sm:grid-cols-4">{Object.entries(asset.masterDecision.brainScores).map(([name, score]) => <div key={name}><p className="eyebrow">{name}</p><p className="mono mt-1 text-xs text-foreground">{score ?? '—'}</p></div>)}</div></section>
      </div>

      <SectionLabel aside={<Badge tone={asset.fundamentalBrain.direction === 'BUY' ? 'positive' : asset.fundamentalBrain.direction === 'UNAVAILABLE' ? 'neutral' : 'amber'}>{asset.fundamentalBrain.direction}</Badge>}>{t('asset.fundamentalBreakdown')}</SectionLabel>
      <section className="mb-6 panel p-5" data-testid="fundamental-brain-panel"><div className="mb-4 flex flex-col justify-between gap-3 border-b border-border pb-4 sm:flex-row sm:items-start"><div><p className="text-sm text-foreground">{asset.fundamentalBrain.rationale}</p><p className="mt-1 text-xs text-muted-foreground">{t('common.confidence')} {asset.fundamentalBrain.confidence}%</p></div><span className="mono text-2xl text-primary">{asset.fundamentalBrain.score ?? '—'}</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{([
        ['pe', t('asset.metric.pe')], ['forwardPe', t('asset.metric.forwardPe')], ['peg', t('asset.metric.peg')], ['epsGrowth', t('asset.metric.epsGrowth')], ['revenueGrowth', t('asset.metric.revenueGrowth')], ['operatingMargin', t('asset.metric.operatingMargin')], ['roe', t('asset.metric.roe')], ['debtToEquity', t('asset.metric.debtToEquity')],
      ] as const).map(([key, label]) => { const metric = asset.fundamentalBrain.metrics[key]; return <div className="rounded-md bg-secondary/40 p-3" key={key}><p className="eyebrow">{label}</p><div className="mt-2 flex items-end justify-between"><p className="mono text-lg text-foreground">{metric.value ?? '—'}</p><Badge tone={metric.label === 'strong' || metric.label === 'attractive' ? 'positive' : metric.label === 'weak' || metric.label === 'expensive' ? 'negative' : 'neutral'}>{metric.label}</Badge></div></div>; })}</div>{asset.fundamentalBrain.warnings.length > 0 && <p className="mt-4 text-xs text-muted-foreground">{asset.fundamentalBrain.warnings.join(' · ')}</p>}</section>
...
export function SimulatorPage() {
  ...
  const [symbol, setSymbol] = useState('QQQ'); const [side, setSide] = useState('Buy'); const [size, setSize] = useState('5000'); const [rationale, setRationale] = useState(''); const [submitted, setSubmitted] = useState(false);
  const createMemory = useCreateDecisionMemory({ mutation: { onSuccess: () => queryClient.invalidateQueries({ queryKey: getGetRecentDecisionMemoryQueryKey() }) } });
  const submit = (event: FormEvent) => {
    event.preventDefault();
    setSubmitted(false);
    createMemory.mutate({ data: { externalId: crypto.randomUUID(), symbol, algorithmVersion: 'V7.2', regime: 'PAPER', decision: side.toUpperCase(), finalScore: 70, confidence: 70, sizeMultiplier: 1, rationale: rationale || t('simulator.defaultRationale'), brainSnapshot: { source: 'simulator', side }, marketSnapshot: { notional: Number(size || 0), mode: 'PAPER' } } }, { onSuccess: () => setSubmitted(true) });
  };
 return <div className="content-wrap"><PageHeader eyebrow={t('simulator.eyebrow')} title={t('simulator.title')} subtitle={t('simulator.subtitle')} action={<Badge tone="amber"><SlidersHorizontal size={11} />{t('simulator.builder')}</Badge>} /><div className="grid gap-5 lg:grid-cols-[.9fr_1.1fr]"><form className="panel p-5 md:p-6" onSubmit={submit}><SectionLabel>{t('simulator.inputs')}</SectionLabel><div className="space-y-5"><label className="block"><span className="eyebrow">{t('simulator.instrument')}</span><select value={symbol} onChange={(e) => setSymbol(e.target.value)} className="mt-2 w-full rounded-md border border-input bg-background px-3 py-3 text-sm text-foreground" data-testid="select-simulator-symbol">{mockMarkets.map((m) => <option value={m.symbol} key={m.symbol}>{m.symbol} — {m.name}</option>)}</select></label><div><span className="eyebrow">{t('simulator.direction')}</span><div className="mt-2 grid grid-cols-2 gap-2">{['Buy', 'Sell'].map((item) => <button type="button" onClick={() => setSide(item)} className={`rounded-md border py-3 text-sm font-semibold ${side === item ? item === 'Buy' ? 'border-accent bg-accent/10 text-accent' : 'border-destructive bg-destructive/10 text-destructive' : 'border-border text-muted-foreground'}`} key={item} data-testid={`button-side-${item.toLowerCase()}`}>{item === 'Buy' ? t('simulator.buy') : t('simulator.sell')}</button>)}</div></div><label className="block"><span className="eyebrow">{t('simulator.notional')}</span><div className="mt-2 flex items-center rounded-md border border-input bg-background px-3"><span className="mono text-muted-foreground">$</span><input value={size} onChange={(e) => setSize(e.target.value)} type="number" min="100" step="100" className="w-full bg-transparent px-2 py-3 mono text-sm text-foreground outline-none" data-testid="input-paper-notional" /></div></label><label className="block"><span className="eyebrow">{t('simulator.why')}</span><textarea value={rationale} onChange={(event) => setRationale(event.target.value)} placeholder={t('simulator.whyPlaceholder')} className="mt-2 min-h-[92px] w-full resize-none rounded-md border border-input bg-background px-3 py-3 text-sm text-foreground outline-none placeholder:text-muted-foreground" data-testid="input-trade-rationale" /></label><button type="submit" disabled={createMemory.isPending} className="w-full rounded-md bg-primary py-3 text-sm font-bold text-primary-foreground transition hover:brightness-110 disabled:opacity-60" data-testid="button-generate-proposal"><Play size={15} className="mr-2 inline" />{createMemory.isPending ? t('simulator.saving') : t('simulator.generate')}</button></div></form><div className="space-y-4"><div className="panel p-5 md:p-6"><SectionLabel aside={<Badge tone="teal">{t('common.paper')}</Badge>}>{t('simulator.preview')}</SectionLabel><div className="flex items-center justify-between border-b border-border py-4"><div><span className="mono text-2xl text-foreground">{symbol}</span><p className="mt-1 text-xs text-muted-foreground">{t('simulator.currentProposal')}</p></div><Badge tone={side === 'Buy' ? 'positive' : 'negative'}>{side === 'Buy' ? t('simulator.buy') : t('simulator.sell')}</Badge></div><div className="grid grid-cols-2 gap-4 py-5"><div><p className="eyebrow">{t('history.notional')}</p><p className="mono mt-2 text-xl text-foreground">${Number(size || 0).toLocaleString()}</p></div><div><p className="eyebrow">{t('simulator.maxLoss')}</p><p className="mono mt-2 text-xl text-primary">${Math.round(Number(size || 0) * .02).toLocaleString()}</p></div></div><Notice tone="teal"><span>{t('simulator.boundedNotice')}</span></Notice>{submitted && <div className="mt-4 flex items-center gap-2 rounded-md border border-accent/25 bg-accent/5 p-3 text-xs text-accent" data-testid="status-proposal-created"><CheckCircle2 size={15} />{t('simulator.savedToMemory')}</div>}{createMemory.isError && <div className="mt-4"><Notice tone="negative"><span>{t('simulator.saveFailed')}</span></Notice></div>}</div><div className="panel p-5"><SectionLabel>{t('simulator.before')}</SectionLabel><ul className="space-y-3 text-xs text-muted-foreground"><li className="flex gap-2"><CheckCircle2 size={14} className="shrink-0 text-accent" />{t('simulator.check1')}</li><li className="flex gap-2"><CheckCircle2 size={14} className="shrink-0 text-accent" />{t('simulator.check2')}</li><li className="flex gap-2"><CheckCircle2 size={14} className="shrink-0 text-accent" />{t('simulator.check3')}</li></ul></div></div></div></div>;
}
...
export function BacktestPage() {
  const { t } = useI18n();
  const query = useGetRecentDecisionMemory({ limit: 200 });
  const records = query.data ?? [];
  const closed = records.filter((record) => record.outcomeR !== null);
  const wins = closed.filter((record) => Number(record.outcomeR) > 0);
  const averageR = closed.length ? closed.reduce((sum, record) => sum + Number(record.outcomeR), 0) / closed.length : 0;
  return <div className="content-wrap"><PageHeader eyebrow={t('backtest.eyebrow')} title={t('backtest.title')} subtitle={t('backtest.subtitle')} action={<Badge tone="teal">{t('axi.paperOnly')}</Badge>} />{query.isLoading ? <StateMessage kind="loading" title={t('backtest.loading')} body="" /> : query.isError ? <StateMessage kind="error" title={t('backtest.unavailable')} body={t('error.desc')} onRetry={() => query.refetch()} /> : <><div className="mb-5 grid gap-3 md:grid-cols-3"><Metric label={t('backtest.decisions')} value={String(records.length)} detail={t('backtest.fromMemory')} /><Metric label={t('backtest.winRate')} value={closed.length ? `${Math.round(wins.length / closed.length * 100)}%` : '—'} detail={`${closed.length} ${t('backtest.closed')}`} tone={wins.length > closed.length / 2 ? 'positive' : 'neutral'} /><Metric label={t('backtest.averageR')} value={closed.length ? `${averageR >= 0 ? '+' : ''}${averageR.toFixed(2)}R` : '—'} detail={t('backtest.paperOutcomes')} tone={averageR > 0 ? 'positive' : averageR < 0 ? 'negative' : 'neutral'} /></div><Notice><span>{t('backtest.memoryNotice')}</span></Notice></>}</div>;
}

export function HistoryPage() {
  const { t } = useI18n();
  const queryClient = useQueryClient();
  const query = useGetRecentDecisionMemory({ limit: 50 });
  const outcome = useUpdateDecisionMemoryOutcome({ mutation: { onSuccess: () => queryClient.invalidateQueries({ queryKey: getGetRecentDecisionMemoryQueryKey() }) } });
  const records = query.data ?? [];
  const close = (externalId: string, outcomeR: number) => outcome.mutate({ externalId, data: { outcomeR, maxAdverseExcursionR: outcomeR > 0 ? -0.35 : -1, maxFavourableExcursionR: outcomeR > 0 ? 1.2 : 0.25, exitReason: outcomeR > 0 ? 'PAPER_TARGET' : 'PAPER_STOP' } });
  return <div className="content-wrap"><PageHeader eyebrow={t('history.eyebrow')} title={t('history.title')} subtitle={t('history.subtitle')} action={<Badge tone="neutral"><HistoryIcon />{records.length} {t('history.proposals')}</Badge>} /><Notice tone="teal"><span>{t('history.persisted')}</span></Notice>{query.isLoading ? <div className="mt-5"><StateMessage kind="loading" title={t('history.loading')} body="" /></div> : query.isError ? <div className="mt-5"><StateMessage kind="error" title={t('history.unavailable')} body={t('error.desc')} onRetry={() => query.refetch()} /></div> : <div className="mt-5 panel overflow-x-auto"><div className="min-w-[760px]"><div className="grid grid-cols-[1.2fr_.8fr_.8fr_.8fr_.8fr_1.5fr] gap-3 border-b border-border bg-secondary/40 px-5 py-3 eyebrow"><span>{t('history.date')}</span><span>{t('history.instrument')}</span><span>{t('history.side')}</span><span>{t('history.score')}</span><span>{t('history.result')}</span><span>{t('history.status')}</span></div>{records.map((record) => <div className="grid grid-cols-[1.2fr_.8fr_.8fr_.8fr_.8fr_1.5fr] items-center gap-3 border-b border-border px-5 py-4 last:border-0" key={record.externalId}><span className="mono text-xs text-foreground">{new Intl.DateTimeFormat(undefined, { dateStyle: 'medium' }).format(new Date(record.createdAt))}</span><span className="mono text-xs text-foreground">{record.symbol}</span><span className="mono text-xs text-foreground">{record.decision.replace('_', ' ')}</span><span className="mono text-xs text-primary">{record.finalScore ?? '—'}</span><span className={`mono text-xs ${record.outcomeR === null ? 'text-muted-foreground' : Number(record.outcomeR) >= 0 ? 'text-accent' : 'text-destructive'}`}>{record.outcomeR === null ? '—' : `${Number(record.outcomeR) >= 0 ? '+' : ''}${Number(record.outcomeR).toFixed(2)}R`}</span><div>{record.closedAt ? <Badge tone={Number(record.outcomeR) >= 0 ? 'positive' : 'negative'}>{t('history.closed')}</Badge> : <div className="flex gap-2"><button disabled={outcome.isPending} onClick={() => close(record.externalId, 1)} className="rounded border border-accent/30 px-2 py-1 text-[10px] font-semibold text-accent">{t('history.markWin')}</button><button disabled={outcome.isPending} onClick={() => close(record.externalId, -1)} className="rounded border border-destructive/30 px-2 py-1 text-[10px] font-semibold text-destructive">{t('history.markLoss')}</button></div>}</div></div>)}{records.length === 0 && <p className="p-6 text-sm text-muted-foreground">{t('history.empty')}</p>}</div></div>}</div>;
}
