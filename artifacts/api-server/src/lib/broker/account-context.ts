...
/**
 * Creates an immutable account context from authenticated identity + explicit
 * account selector. Never accepts userId from query/body/headers.
 */
export function getTradingAccountContext(req: Request): TradingAccountRequestContext {
  ...
  if (!/^[A-Za-z0-9_-]{6,128}$/.test(tradingAccountId)) {
    throw new AccountIsolationError("Invalid trading account identifier.");
  }
